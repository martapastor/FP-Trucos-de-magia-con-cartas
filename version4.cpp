...
bool cortar(tMazo mazo, int cuantas);
...
int main() {
	...
	while (opcionMenu != 0) {
		if (opcionMenu == 1) {
			...
			cin >> cuantas;
			bool cortarMazo = cortar (mazo, cuantas);
			if (cortarMazo == true) mostrar (mazo);
		}
			...
			cout << "REPARTIR DE MANERA ALTERNA EN 3 MONTONES" << endl;
			tMazo monton;
			...
			cout << "TRUCO DE LA POSADA" << endl;
			trucoPosada(mazo, nomb);
		}
		opcionMenu = menu ();
	}
...
int menu()	{
	...
	cout << "11 - Truco de la posada" << endl;
...
bool unir(tMazo mazo, const tMazo otroMazo) {
	...
	if (total < 52) {
		...
		cout << "Error: no ha podido añadirse el mazo seleccionado por superar el límite de cartas total." << endl;
		noMaxCartas = false;
	}
...
bool cortar(tMazo mazo, int cuantas) {
	...
	if (puedePartirse == true) {
		bool puedenUnirse = unir (mazo, mazoAux);
		return true;
	}
	else {
		cout << "Error: el mazo no puede cortarse por la posición indicada." << endl;
		return false;
	}
}	
...
void trucoTresMontones(tMazo mazo, string &nomb) {
	...
	if (cargarMazo == true) {
		...
		while (cont < 3) {
			...
			while (queMazo == dondeEsta) queMazo = (rand() % 3) + 1;
			int mazoUno = queMazo;
			repartirIntercalando (mazo, 3, mazoUno, monton1);
			repartirIntercalando (mazo, 3, dondeEsta, monton2);

			queMazo = (rand() % 3) + 1;
			while (queMazo == mazoUno || queMazo == dondeEsta) queMazo = (rand() % 3) + 1;
...
void trucoPosada(tMazo mazo, string &nomb) {
	...
	if (cargarMazo == true) {
		int i = 0;
		while (((mazo[i] - 1) % CARTASxPALO) == 10) i++;
		while (((mazo[i] - 1) % CARTASxPALO) == 11) i++;
		while (((mazo[i] - 1) % CARTASxPALO) == 12) i++;
		while (((mazo[i] - 1) % CARTASxPALO) == 0) i++;
				
		bool mazoCorrecto;
		if (i == 16) mazoCorrecto = true;
		else mazoCorrecto = false;

		if (mazoCorrecto == true) {
			tMazo monton1, monton2, monton3, monton4;
			repartirMontones (mazo, 4, monton4); //Al acabar de repartir los 4 montones, el que se guarda es el montón4, por lo que no hace falta volver a repartirlo y a guardarlo		
			repartirIntercalando (mazo, 4, 1, monton1);
			repartirIntercalando (mazo, 4, 2, monton2);
			repartirIntercalando (mazo, 4, 3, monton3);

			unir (monton1, monton2);
			unir (monton1, monton3);
			unir (monton1, monton4);

			vacio(mazo);
			unir (mazo, monton1);

			int cuantas;
			cout << "\nIntroduce la posición por la que quieres cortar el mazo: ";
			cin >> cuantas;
			bool cortarMazo = cortar (mazo, cuantas);

			repartirMontones (mazo, 4, monton1); //Ahora ya no importa en qué montón se guarden las últimas cartas que se reparten alternamente
		}
		else  
			cout << "Error: el truco no ha podido ejecutarse. El mazo no es válido." << endl;
